<template>
  <div class="app-container">
    <el-card>
      <div slot="header">
        <span>报到流程管理</span>
        <el-button style="float: right;" type="primary" size="small" @click="handleAdd">新增流程</el-button>
      </div>
      ...
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="流程类型">
          <el-select v-model="form.processType" style="width: 100%;">
            <el-option label="报到注册" value="报到注册" />
            <el-option label="缴费" value="缴费" />
            <el-option label="宿舍分配" value="宿舍分配" />
        ...
        <el-form-item label="完成时间">
          <el-date-picker v-model="form.completeTime" type="datetime" style="width: 100%;" />
        </el-form-item>
...
export default {
  ...
  methods: {
    loadData() {
      this.loading = true
      request.get('/welcome/process/list', {
        params: {
          ...this.queryParams,
    ...
    handleAdd() {
      this.dialogTitle = '新增报到流程'
      this.isEdit = false
      this.form = { status: '未完成' }
      this.dialogVisible = true
    },
    handleEdit(row) {
      this.dialogTitle = '编辑报到流程'
      this.isEdit = true
      this.form = { ...row }
      this.dialogVisible = true
    },
    handleDelete(row) {
      this.$confirm('确认删除该流程？', '提示', { type: 'warning' }).then(() => {
        request.delete(`/welcome/process/${row.id}`).then(() => {
    ...
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          api('/welcome/process', this.form).then(() => {
