  ...
  <div class="app-container">
    <el-card>
      ...
      <el-table :data="tableData" v-loading="loading" border>
        ...
        <el-table-column prop="vision" label="视力" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="视力">
          <el-input v-model="form.vision" placeholder="如：5.0" />
        </el-form-item>
        ...
        <el-form-item label="疾病史">
          <el-input type="textarea" v-model="form.diseaseHistory" rows="3" />
        </el-form-item>
        <el-form-item label="过敏史">
          <el-input type="textarea" v-model="form.allergyHistory" rows="3" />
        </el-form-item>
...
export default {
  ...
  methods: {
    loadData() {
      this.loading = true
      request.get('/student-work/health/list', {
        params: {
          ...this.queryParams,
    ...
    handleDelete(row) {
      this.$confirm('确认删除该记录？', '提示', { type: 'warning' }).then(() => {
        request.delete(`/student-work/health/${row.id}`).then(() => {
    ...
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          api('/student-work/health', this.form).then(() => {
